...
public class WatchlistController {
    ...
    @DeleteMapping("/{groupId}") public ApiResponse<Void> delete(@CurrentUser AuthenticatedUser user,@PathVariable("groupId") String groupId,@RequestParam("version") long version,HttpServletRequest request){useCase.delete(user,groupId,version);return ApiResponse.success(RequestIdFilter.get(request),null);}
    @PostMapping("/{groupId}/items") public ApiResponse<WatchlistGroup> add(@CurrentUser AuthenticatedUser user,@PathVariable("groupId") String groupId,@Valid @RequestBody ItemBody body,HttpServletRequest request){return ApiResponse.success(RequestIdFilter.get(request),useCase.add(user,groupId,body.fundCode(),body.note(),body.tags()));}
    @PatchMapping("/{groupId}/items/{itemId}") public ApiResponse<WatchlistGroup> updateItem(@CurrentUser AuthenticatedUser user,@PathVariable("groupId") String groupId,@PathVariable("itemId") String itemId,@Valid @RequestBody ItemPatch body,HttpServletRequest request){return ApiResponse.success(RequestIdFilter.get(request),useCase.updateItem(user,groupId,itemId,body.note(),body.tags(),body.version()));}
    @DeleteMapping("/{groupId}/items/{itemId}") public ApiResponse<WatchlistGroup> removeItem(@CurrentUser AuthenticatedUser user,@PathVariable("groupId") String groupId,@PathVariable("itemId") String itemId,@RequestParam("version") long version,HttpServletRequest request){return ApiResponse.success(RequestIdFilter.get(request),useCase.removeItem(user,groupId,itemId,version));}
