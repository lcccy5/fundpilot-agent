...
export default function AccountPage(){
  const[questions,setQuestions]=useState<Question[]>([]);const[questionsReady,setQuestionsReady]=useState(false);const[questionsFailed,setQuestionsFailed]=useState(false);const[answers,setAnswers]=useState<Record<string,number>>({});const[profile,setProfile]=useState<Profile|null>(null);const[profileReady,setProfileReady]=useState(false);const[saving,setSaving]=useState(false);const[attempted,setAttempted]=useState(false);const[notice,setNotice]=useState('风险画像来自问卷评分，用于帮助理解研究内容');
  ...
  const unansweredId=firstUnanswered(questions,answers);
  const missingId=profile&&!attempted?null:unansweredId;
