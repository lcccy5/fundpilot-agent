...
export default function WatchlistsPage(){
  ...
  useEffect(()=>{api<Group[]>('/api/v1/watchlists').then(next=>{setGroups(next);showProfiles(next);const count=next.reduce((sum,group)=>sum+group.items.length,0);setNotice(count?`已加载 ${count} 只自选基金`:'还没有自选基金，先添加一只吧');}).catch(x=>setNotice(x instanceof Error?x.message:'请先登录后查看云端自选')).finally(()=>setLoading(false));},[]);

  const create=async(e:FormEvent)=>{e.preventDefault();const name=groupName.trim();if(!name)return setNotice('请填写分组名称');try{const created=await api<Group>('/api/v1/watchlists',{method:'POST',body:JSON.stringify({name})});setGroups(current=>[...current,created]);setGroupName('');setNotice(`已创建“${created.displayName}”分组`);}catch(x){setNotice(x instanceof Error?x.message:'创建分组失败');}};
  ...
  return <AppShell notice={notice}><section className="workspace watchlist-page">
    ...
    <details className="create-group-panel"><summary>＋ 新建自选分组</summary><form onSubmit={create}><label>分组名称<input value={groupName} onChange={e=>setGroupName(e.target.value)} placeholder="例如：长期关注、机器人主题"/></label><button>创建分组</button></form></details>
  </section></AppShell>;
}
